...
class ConfigManager:
            ...
            if env_var in os.environ:
                ...
                if config_path[0] == "processing" and config_path[1] == "batch_size":
                    try:
                        ...
                        logger.warning(
                            f"Invalid value for {env_var}: {value}. Using default."
                        )
                        continue
    ...
    def _validate_config(self) -> None:
        ...
        if self.config["llm"]["provider"].lower() not in valid_providers:
            logger.warning(
                f"Invalid LLM provider: {self.config['llm']['provider']}. Using default: groq"
            )
            self.config["llm"]["provider"] = "groq"

        # Validate batch size
        if (
            not isinstance(self.config["processing"]["batch_size"], int)
            or self.config["processing"]["batch_size"] < 1
        ):
            logger.warning(
                f"Invalid batch size: {self.config['processing']['batch_size']}. Using default: 5"
            )
    ...
    def get_api_key(self) -> str:
        ...
        if not api_key:
            ...
            raise ValueError(
                f"API key not found. Please set {env_var_name} environment variable."
            )
