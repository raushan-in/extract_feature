...
class BatchProcessor:
    ...
    def process_files(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Failed to find files matching pattern {self.file_pattern}: {str(e)}")
            raise
            ...
            with tqdm(total=len(futures), desc="Processing files", unit="file") as progress_bar:
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        logger.error(f"Unexpected error processing {os.path.basename(file_path)}: {str(e)}")
                        results.append({
        ...
        logger.info(f"Processing complete: {success_count}/{len(results)} files successful ({success_rate:.1%})")
